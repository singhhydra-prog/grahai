...
/* ════════════════════════════════════════════════════════
   Product 4 — AI Astrologer (Overlay Component)

   Beautiful, standalone "Ask the AI Astrologer" experience
   converted to an overlay component.
...
function computeMoonLong(jd: number): number {
  ...
  const M = norm360(134.963 + 477198.8675 * T) * DEG
  const D = norm360(297.850 + 445267.1114 * T) * DEG
  const F = norm360(93.272 + 483202.0181 * T) * DEG
  const C = 6.289 * Math.sin(M) + 1.274 * Math.sin(2 * D - M) + 0.658 * Math.sin(2 * D)
  return norm360(L + C)
}
...
function computeMarsLong(jd: number): number {
  const T = (jd - 2451545.0) / 36525
  const L = norm360(335.194 + 19140.3023 * T)
  const M = norm360(17.955 + 19139.3863 * T) * DEG
  const C = 1.919 * Math.sin(M) + 0.028 * Math.sin(2 * M)
  return norm360(L + C)
}
...
function computeMercuryLong(jd: number): number {
  const T = (jd - 2451545.0) / 36525
  const L = norm360(100.464 + 149474.2238 * T)
  const M = norm360(80.769 + 149475.3956 * T) * DEG
  const C = 3.207 * Math.sin(M) + 0.041 * Math.sin(2 * M)
  return norm360(L + C)
}
...
function computeJupiterLong(jd: number): number {
  const T = (jd - 2451545.0) / 36525
  const L = norm360(14.331 + 3034.9057 * T)
...
export default function AstrologerView({ onBack, onUpgrade }: { onBack: () => void; onUpgrade: () => void }) {
    ...
    return (
      <div className="min-h-screen bg-bg flex flex-col">
        ...
        <div className="sticky top-0 z-50 flex items-center gap-3 px-4 py-3 bg-[#0a0a0f]/90 backdrop-blur-xl border-b border-white/[0.04]">
          ...
          <h1 className="text-lg font-bold text-white">AI Astrologer</h1>
        </div>
        ...
        <div className="relative z-10 flex-1 flex flex-col justify-center max-w-2xl mx-auto px-6 w-full pb-16">
          {/* Header */}
          <motion.div
  ...
  return (
    <div className="min-h-screen bg-bg flex flex-col">
      ...
      {/* Messages */}
      <div className="flex-1 pt-4 pb-32 overflow-y-auto">
      ...
      <div className="fixed bottom-0 w-full bg-gradient-to-t from-bg via-bg/95 to-transparent pt-6 pb-6">
        <div className="max-w-2xl mx-auto px-6">
          ...
          <div className="flex items-center justify-center gap-4 mt-3">
            <button className="text-[9px] text-text-dim/30 hover:text-gold/40 transition-colors tracking-wider">
              Daily Horoscope
            </button>
            <button className="text-[9px] text-text-dim/30 hover:text-gold/40 transition-colors tracking-wider">
              Birth Chart
