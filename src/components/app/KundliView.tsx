...
function KundliResults({ result, details, onAskAI }: { result: KundliResult; details: BirthDetails; onAskAI: () => void }) {
...
export default function KundliView({ onBack, onAskAI }: { onBack: () => void; onAskAI: () => void }) {
  ...
  // Auto-detect birth data from onboarding — skip form if already provided
  useEffect(() => {
    if (autoSubmitted.current) return
    try {
      const saved = localStorage.getItem("grahai-onboarding-birthdata")
      if (saved) {
        const bd = JSON.parse(saved) as { date?: string; time?: string; place?: string; timeUnknown?: boolean; lat?: number; lng?: number; tz?: number }
        if (bd.date) {
          autoSubmitted.current = true
          const birthDetails: BirthDetails = {
            name: "",
            date: bd.date,
            time: bd.timeUnknown ? "12:00" : (bd.time || "12:00"),
            timeUnknown: bd.timeUnknown || false,
            city: bd.place || "",
            country: "India",
            lat: bd.lat,
            lng: bd.lng,
          ...
          handleSubmit(birthDetails)
        }
      }
    } catch (e) {
      // localStorage unavailable or corrupted — show form as fallback
    }
  ...
  return (
    <main className="min-h-screen bg-[#050810]">
      <div className="sticky top-0 z-50 flex items-center gap-3 px-4 py-3 bg-[#0a0a0f]/90 backdrop-blur-xl border-b border-white/[0.04]">
        ...
        <h1 className="text-lg font-bold text-white">Birth Chart (Kundli)</h1>
      </div>
      ...
      <div className="mx-auto max-w-4xl px-4 py-8">
        ...
        {stage === "result" && result && details && (
          <KundliResults result={result} details={details} onAskAI={onAskAI} />
        )}
      </div>
    </main>
  )
