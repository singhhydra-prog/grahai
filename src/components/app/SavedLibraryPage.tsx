        ...
        <div className="max-w-lg mx-auto px-4 py-3">
          <div className="flex items-center gap-3 mb-3">
            ...
            <div>
              ...
              <p className="text-xs text-[#5A6478]">Your answers, reports, and matches</p>
            </div>
          </div>
      ...
      <div className="max-w-lg mx-auto px-4 py-4 space-y-4 pb-12">
        ...
        {activeTab === "answers" && (
          <div className="flex items-center gap-2 rounded-xl border border-white/5 bg-white/[0.02] px-3 py-2.5">
            <Search className="w-4 h-4 text-[#5A6478]" />
            <input
              type="text"
              value={searchQuery}
            ...
            >
              {filteredAnswers.length === 0 ? (
                <div className="text-center py-12">
                  <Bookmark className="w-10 h-10 text-[#5A6478]/30 mx-auto mb-3" />
                  <p className="text-sm text-[#5A6478]">No saved answers yet</p>
                  <p className="text-xs text-[#5A6478]/60 mt-1">Ask GrahAI a question to get started</p>
                </div>
              ) : (
                  ...
                  <div key={answer.id} className="rounded-xl border border-white/5 bg-white/[0.02] p-4">
                    <div className="flex items-start justify-between mb-2">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-[10px] text-[#5A6478]">{timeAgo(answer.timestamp)}</span>
                      </div>
                      <button className="p-1 hover:bg-white/5 rounded-lg transition-colors">
                        <Trash2 className="w-3.5 h-3.5 text-[#5A6478]" />
                      </button>
                    </div>
                    <p className="text-sm font-medium text-[#F1F0F5] mb-1">{answer.question}</p>
                    <p className="text-xs text-[#8A8F9E] leading-relaxed line-clamp-2">{answer.excerpt}</p>
          ...
          {activeTab === "reports" && (
            ...
            >
              {MOCK_REPORTS.length === 0 ? (
                <div className="text-center py-12">
                  <FileText className="w-10 h-10 text-[#5A6478]/30 mx-auto mb-3" />
                  <p className="text-sm text-[#5A6478]">No reports yet</p>
                  <p className="text-xs text-[#5A6478]/60 mt-1">Purchase or generate a report to see it here</p>
                </div>
              ) : (
                  ...
                  <div key={report.id} className="rounded-xl border border-white/5 bg-white/[0.02] p-4 flex items-center gap-3">
                    ...
                    <div className="flex-1">
                      ...
                      <div className="flex items-center gap-2 mt-0.5">
                        ...
                        <span className="text-[10px] text-[#5A6478]">{timeAgo(report.timestamp)}</span>
                      </div>
                    </div>
          ...
          {activeTab === "compatibility" && (
            ...
            >
              {MOCK_COMPAT.length === 0 ? (
                <div className="text-center py-12">
                  <Heart className="w-10 h-10 text-[#5A6478]/30 mx-auto mb-3" />
                  <p className="text-sm text-[#5A6478]">No compatibility checks yet</p>
                  <p className="text-xs text-[#5A6478]/60 mt-1">Check compatibility with someone to see results here</p>
                </div>
              ) : (
                    ...
                    <div key={compat.id} className="rounded-xl border border-white/5 bg-white/[0.02] p-4 flex items-center gap-3">
                      ...
                      <div className="flex-1">
                        ...
                        <div className="flex items-center gap-2 mt-0.5">
                          ...
                          <span className="text-[10px] text-[#5A6478]">{timeAgo(compat.timestamp)}</span>
                        </div>
                      </div>
                      <ChevronRight className="w-4 h-4 text-[#5A6478]" />
                    </div>
